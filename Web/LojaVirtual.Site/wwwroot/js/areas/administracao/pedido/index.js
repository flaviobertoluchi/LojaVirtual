$(function () {
    $('.cpfBR').mask('000.000.000-00', { reverse: true, clearIfNotMatch: true });
    ...
    $('.dataCompraInicial').on('change', function () {
        obterPaginado(1);
    });

    $('.dataCompraFinal').on('change', function () {
        obterPaginado(1);
    });

    function obterPaginado(pagina) {
        $.get('pedidos/paginacao'
            + '?pagina=' + pagina
            ...
            + '&dataCompraInicial=' + $('.dataCompraInicial').val()
            + '&dataCompraFinal=' + $('.dataCompraFinal').val())
